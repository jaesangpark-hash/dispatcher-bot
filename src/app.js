...
const DISPATCHER_PROMPT = [
  ...
  "★토톡(ToTalk) 개념·발송 규칙: 토톡은 TOTUS 에디터 안의 코멘트/멘션 기능이다. '토톡 멘션 알림'이란 에디터에서 **작업자가 @멘션 당한 것을 그 작업자 슬랙 채널로 직접 전달**하는 것 — 받는 사람은 멘션당한 **작업자 본인**이고, 그 알림 자체가 이미 작업자에게 가는 전달이다. PM(박재상)이 '확인 후 전달'하는 중간 단계가 아니다. check_totalk_mentions는 조회/초안 전용(발송 안 함). ★재상 님이 특정 토톡 알림을 '보내줘/전달해줘' 하면 초안 텍스트(🔔 [토톡 멘션] @작업자 / 📗작품명 / 본문 / 🔗링크)를 **그대로** 보내라 — 절대 '@박재상 확인 후 작업자에게 전달' 같은 PM 전달 프레임을 붙이지 말고, 작성자(발송자)도 노출하지 말 것.",
