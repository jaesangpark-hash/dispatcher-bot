...
const DISPATCHER_PROMPT = [
  ...
  "- 설정집 AI검수 인증번호 발급/재발급('OO 인증번호 발급해줘', '배정 끝났으니 토큰 발급해줘', 한도 초과로 폐기된 뒤 다시 필요할 때): reissue_setjip_ai_token(pivo, [role], [worker], [thread]). ★role 생략하면 TOTUS에서 그 작품 '설정집' JOB의 번역(OTC0052)·번역검수(OTC0054) 담당자를 실시간 조회해 배정된 역할 전부를 한 번에 발급(배정현황 구글시트는 하루 1번만 동기화라 방금 배정한 건 못 잡을 수 있어 TOTUS를 우선 씀, 못 찾으면 시트로 폴백) — 'APM이 배정 다 됐다고 알려주며 토큰 요청'하는 흔한 경우가 이거다. 특정 역할 하나만 원하면 role 지정. worker는 자동조회를 못 믿을 때만 수동 지정. 이메일이 작업자 DB에 있으면 결과 메시지에 Slack 멘션도 자동으로 붙는다. 설정집 작성 요청 스레드 안에서 부르면 thread 생략, 그 스레드(공개 채널)에 바로 게시 — DM 아님. 게이트 없이 즉시 실행(발급은 되돌리기 쉬운 저위험 동작).",
      ...
      async ({ pivo, role, worker, thread }) => {
        try {
          ...
          for (const r of roles) {
            ...
            results.push({ role: r, worker: finalWorker, email, slackId: slackInfo?.slackId || "", code: issued.code, limit: issued.limit });
          }
          ...
          if (channel && ts) {
            const lines = results.map((it) => {
              ...
              return `• ${mention}${it.role}(${it.worker}) : \`${it.code}\` (${it.limit}회)`;
            });
