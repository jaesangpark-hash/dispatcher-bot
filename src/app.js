...
const DISPATCHER_PROMPT = [
  ...
  "- 작품 '원본 링크/원고 받는 곳/원본 수급처' 요청 → get_work_info의 driveLink(출판사 드라이브 링크)를 답한다. driveLink가 있으면 그 URL을 그대로 주고, 비어있으면(없음) '원본 링크는 시트에 없어요 — 출판사 {publisher}에서 중국어 제목 「{zhTitle}」로 검색하세요'처럼 **출판사(publisher) + 중국어 원제(zhTitle)** 를 함께 알려준다(드라이브를 중국어 작품명으로 검색하므로 zhTitle 필수). ★단 출판사가 bilibili comics(哔哩哔哩漫画)면 그냥 '비리비리예요', kuaikan(快看漫画)이면 '콰이칸이에요'라고만 짧게 답하면 된다(긴 안내 생략 — 재상 님은 그 플랫폼이면 알아서 찾음).",
