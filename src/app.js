...
const SETJIP_REF_KEYWORDS = ["원본", "체크리스트", "재수급", "原本", "チェック"];
...
const DISPATCHER_PROMPT = [
  ...
  "★도구 라우팅(엄수·양방향 폴백 금지): ①운영·내부 데이터(작품·납품·일정·작업자·정산·고객사·스케줄 등)는 반드시 내부 도구(get_*·query_sheet·totus_*·read_tab·query_schedule 등)로만 조회한다. 못 찾으면 '못 찾았다'고 답하고 작품명 표기 확인을 요청한다 — 절대 웹으로 넘어가지 마라. ②WebSearch(웹 검색)는 사용자가 '웹에서/검색해줘'라고 명시했거나, 환율·일반상식·뉴스처럼 내부에 있을 리 없는 외부·실시간 정보일 때만 쓴다. 웹에서 못 찾으면 '웹에서 못 찾았다'고 답하고 내부 도구로 폴백하지 마라. ③즉 각 요청은 지정된 한쪽 출처에서만 처리하고, 미스는 '못 찾음'으로 끝낸다(반대편으로 안 넘어감). WebFetch(임의 URL 회수)는 쓰지 말고, 같은 검색을 2회 넘게 재시도하지 마라. ④★이 WebFetch 금지는 일반 웹페이지 얘기지 구글 시트 URL/ID엔 해당 안 됨 — 사용자가 구글 시트 링크를 붙이면 '외부 링크라 못 읽는다'고 지레짐작으로 거절하지 말고 read_tab(sheet에 그 URL/ID 그대로)을 실제로 먼저 시도해라(SA가 접근 가능하면 등록 안 된 시트도 읽힌다). 시도해서 실제로 실패했을 때만(권한 없음 등) 안 된다고 답해라 — 안 해보고 못 한다고 답하지 마라. 이 원칙은 시트 링크뿐 아니라 '이거 할 수 있어?' 류의 능력 질문 전반에 적용: 애매하면 먼저 실제로 시도해보고, 그 결과로 가능/불가를 답한다.",
...
async function issueSetjipAiToken({ translator, role, pivo, work, submitDate }) {
  ...
  return { code, limit: SETJIP_AI_TOKEN_DEFAULT_LIMIT, usedCount: 0, reused: false };
}
  ...
  tools: [
    ...
    tool("setjip_reference_files",
      "설정집 에디터에 등록된 참조파일(#번호 그룹 — 원본 체크리스트, 롤이미지, 식자 폰트표 등)을 조회하고, 원하면 슬랙 스레드에 첨부한다. pivo(또는 projectUuid) 필수. attach:false(기본)면 목록만 돌려주고, attach:true면 '원본·체크리스트' 계열 파일만 골라 스레드에 올린다(제출희망일 리마인드가 자동으로 하는 것과 같은 동작). only_keywords로 다른 키워드를 줄 수도 있다. 게이트 없이 즉시 실행(읽기성 공유).",
      {
        ...
        only_keywords: z.array(z.string()).optional().describe("첨부 대상 파일명 키워드(생략 시 원본/체크리스트/재수급/原本/チェック)"),
      },
    ...
    tool("reissue_setjip_ai_token",
      "설정집 AI검수 툴(自己検査) 인증번호를 발급/재발급한다('OO 인증번호 발급해줘/재발급해줘', '배정 끝났으니 토큰 발급해줘'). 한도 초과로 폐기된 뒤 다시 필요할 때, 또는 설정집 작성 요청 확인 시점엔 아직 배정이 안 잡혀 있어서 못 준 역할을 나중에(배정 완료 후) 발급할 때 사용. ★role을 생략하면 TOTUS에서 실시간으로 그 작품의 '설정집' JOB 안 번역(OTC0052)·번역검수(OTC0054) 담당자를 둘 다 조회해서, 배정된 역할만 한 번에 발급한다(아직 배정 안 된 역할은 스킵하고 보고) — '배정현황' 구글시트는 하루 1번만 동기화되어 방금 배정한 게 안 잡힐 수 있으니, worker를 안 주면 항상 이 TOTUS 실시간 조회를 먼저 쓰고, 그래도 못 찾으면 배정현황 시트로 폴백한다. 조회된 이메일이 작업자 DB 시트에 있으면 Slack 멘션도 같이 붙는다. 설정집 작성 요청 스레드 안에서 부르면 thread 생략 가능(그 스레드=공개 채널에 바로 게시, DM 아님). 게이트 없이 즉시 실행(발급은 되돌리기 쉬운 저위험 동작).",
      {
        pivo: z.string().describe("작품 PIVO 번호(숫자만)"),
        role: z.enum(["번역", "번역검수"]).optional().describe("특정 역할 하나만 발급하고 싶을 때만 지정. 생략하면 TOTUS 조회로 배정된 역할 전부(번역+번역검수) 발급 시도"),
        worker: z.string().optional().describe("작업자명 직접 지정(생략하면 TOTUS 실시간 조회 → 안 되면 배정현황 시트 순으로 자동 조회). role 생략 시엔 무시됨(역할별로 자동 조회하므로)"),
      ...
      async ({ pivo, role, worker, thread }) => {
        try {
          ...
          const roles = role ? [role] : ["번역", "번역검수"];
          let totusWorkers = null;
          if (!worker) totusWorkers = await resolveSetjipWorkersFromTotus(pivoNum).catch(() => null);

          const results = [];
          const skipped = [];
          for (const r of roles) {
            let finalWorker = role ? (worker || "").trim() : "";   // role 지정+worker 지정인 단건 케이스만 수동값 사용
            let email = "";
            if (!finalWorker) {
              const fromTotus = totusWorkers && !totusWorkers.error ? (r === "번역" ? totusWorkers.translator : totusWorkers.reviewer) : null;
              if (fromTotus?.name) { finalWorker = fromTotus.name; email = fromTotus.email || ""; }
            }
            if (!finalWorker) {   // TOTUS에 아직 없으면 배정현황 시트로 폴백
              const assign = await lookupAssignment(pivoNum);
              finalWorker = r === "번역" ? assign.translator : assign.reviewer;
            }
            if (!finalWorker) { skipped.push(r); continue; }

            const issued = await issueSetjipAiToken({ translator: finalWorker, role: r, pivo: pivoNum, work, submitDate });
            const slackInfo = email ? await lookupSlackIdByEmail(email).catch(() => null) : null;
            results.push({ role: r, worker: finalWorker, email, slackId: slackInfo?.slackId || "", code: issued.code, limit: issued.limit, usedCount: issued.usedCount, reused: issued.reused });
          }

          if (!results.length) {
            return { content: [{ type: "text", text: JSON.stringify({ error: `발급 가능한 역할이 없음 — TOTUS(설정집 JOB OTC0052/54)·배정현황 시트 둘 다 담당자 미확인(${skipped.join(",")}). 배정 확정 후 다시 시도하거나 worker 인자로 이름을 직접 줘라.` }) }] };
          }

          if (channel && ts) {
            const lines = results.map((it) => {
              const mention = it.slackId ? `<@${it.slackId}> ` : "";
              const status = it.reused ? `(기존 토큰, ${it.usedCount}/${it.limit}회 사용)` : `(${it.limit}회)`;
              return `• ${mention}${it.role}(${it.worker}) : \`${it.code}\` ${status}`;
            });
            const skipNote = skipped.length ? `\n※ ${skipped.join(",")} 담당자는 아직 배정 미확인 — 배정 후 다시 요청해줘.` : "";
            await ctx.client.chat.postMessage({
              channel, thread_ts: ts, ...SENDER,
              text: `🔑 *AI検査 認証番号*\n${lines.join("\n")}\n設定集の自己検査ツールに入力してご利用ください。${skipNote}`,
            }).catch((e) => console.error("[reissue_setjip_ai_token] 게시 실패:", e?.message ?? e));
          }
          return { content: [{ type: "text", text: JSON.stringify({ issued: true, pivo: pivoNum, work, results, skipped, posted: !!(channel && ts) }) }] };
        } catch (e) { return { content: [{ type: "text", text: JSON.stringify({ error: String(e?.message ?? e) }) }] }; }
      },
      { annotations: { readOnlyHint: false } }),
    tool("send_message",
...
async function handleWorkerFileOrder({ message, client }) {
  ...
  // 툰식이를 멘션한 메시지에만 반응한다. 그 외에는 무슨 말이 오가든 끼어들지 않는다.
  // (2026-09-18 森下 님 제보: 무관한 잡담에도 안내문이 나갔다. 원인은 의도 판정이 아니라
  //  "의도가 아니면 안내문"이라는 폴백이었고, 애초에 멘션 없이 전부 받고 있었다.)
  const mentioned = Boolean(SELF_BOT_USER && raw.includes(`<@${SELF_BOT_USER}>`));
  if (!mentioned) return true;   // 멘션했을 때만 반응한다(재상 님 지시, 2026-09-18)
  const text = raw.split(`<@${SELF_BOT_USER}>`).join(" ").trim();
  if (wfoIsHelp(text)) {
...
// 설정집 SETUP 작업자(번역/검수) 실시간 조회 — "배정 현황" 시트(1일 1회 동기화)보다 이게 항상 최신.
// PIVO → projectUuid → "설정집" JOB → OTC0052(정규번역셋업)=번역 담당 / OTC0054(체크번역셋업)=검수 담당
// 재상 님 확인(2026-08-21): 설정집 작성 요청 시점엔 아직 배정 전이라 배정현황에 없는 경우가 잦음 →
// 배정이 실제로 잡힌 뒤 이 조회로 바로 최신값을 가져와 토큰을 발급한다.
async function resolveSetjipWorkersFromTotus(pivo) {
  const rp = await resolveTotusProject({ pivo });
  if (rp?.notFound || rp?.ambiguous) return { error: rp.msg || "TOTUS 프로젝트를 못 찾음" };
  const jobs = await projectJobs(rp.projectUuid).catch(() => null);
  const job = (jobs?.data || []).find((j) => String(j["JOB명"] || "").trim() === "설정집");
  if (!job?.uuid) return { error: "이 작품의 '설정집' JOB을 TOTUS에서 못 찾음(아직 생성 전일 수 있음)" };
  const pick = async (code) => {
    const res = await taskList({ jobUuids: job.uuid, operationTypeCode: code }).catch(() => null);
    const tasks = Array.isArray(res?.data) ? res.data : [];
    const t = tasks.find((x) => x?.작업자?.이메일);
    return t?.작업자 ? { name: t.작업자.이름 || "", email: t.작업자.이메일 || "" } : null;
  };
  const [translator, reviewer] = await Promise.all([pick("OTC0052"), pick("OTC0054")]);
  return { translator, reviewer, projectUuid: rp.projectUuid };
}
// 작업자 DB 시트에서 이메일로 Slack ID 조회 — TOTUS 이메일 컬럼(E, totus_email) 우선, 안 맞으면 일반 이메일(B) 폴백.
async function lookupSlackIdByEmail(email) {
  if (!email) return null;
  try {
    const rows = await readRangeRO(process.env.WORKER_SHEET_ID, process.env.WORKER_SHEET_RANGE || "작업자 DB!A:F");
    const q = String(email).trim().toLowerCase();
    const row = (rows || []).find((r) => String(r[4] || "").trim().toLowerCase() === q)
      || (rows || []).find((r) => String(r[1] || "").trim().toLowerCase() === q);
    return row ? { name: row[0] || "", slackId: row[2] || "", channelId: row[3] || "" } : null;
  } catch (e) { console.error("[lookupSlackIdByEmail] 실패:", e?.message ?? e); return null; }
}

// 설정집 작성 enrich — 견적 + 내부시트(Piccoma 중일ST_v2)에서 자동값 추출(n8n 모달빌드 로직 이식).
...
async function appendDeliveryRows({ publisher, work, pm, apm, order, deliveryYMD, episodes }) {
  ...
  const check = (await readRangeRO(DID, `${TAB}!A${start}:G${start + n - 1}`)) || [];
  const occupied = check.some((r) => (r || []).some((c) => String(c ?? "").trim() !== ""));
  if (occupied) return { ok: false, msg: `안전장치: ${start}~${start + n - 1}행에 이미 데이터가 있어서 쓰기를 중단함(범위 재확인 필요 — 시트가 5만 행을 넘었을 수 있음).` };
  const updates = [];
...
(async () => {
  // ★app.start() 앞에서 확보한다 — 멘션 판정(작업자 채널)이 이 값에 걸려 있어서,
  //   연결 후에 채우면 기동 직후 들어온 멘션을 놓친다.
