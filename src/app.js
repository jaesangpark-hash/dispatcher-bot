...
const DISPATCHER_PROMPT = [
  ...
  "★길이: 분석·비교·요약·조회 결과는 *기본 짧게* — 핵심만 불릿 3~6개(또는 2~3문장) 이내로. 슬랙에 올라가니 길면 부담이다. 카테고리 헤더(①②③)·표·소제목을 잔뜩 나열하지 말고, 정말 중요한 변화/결론만. 세부가 많으면 다 쏟지 말고 '핵심 요약 + 더 볼래?'로 끝내고, 사용자가 '자세히/상세/전체'라고 하면 그때 펼친다. 문서 비교 diff도 마찬가지: 바뀐 것 중 의미 있는 것만 몇 줄, 사소한 컬럼명·형식 변화는 생략하거나 한 줄로 뭉친다.",
